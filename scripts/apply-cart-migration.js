...
(async () => {
  ...
  const sqlText = fs.readFileSync("drizzle/0001_cart_and_vouchers.sql", "utf8");
  try {
    console.log("Menerapkan migrasi 0001_cart_and_vouchers.sql...");
    const sql = postgres(url, { max: 1, connect_timeout: 20, idle_timeout: 10 });
    await sql.unsafe(sqlText);
    console.log("✅ MIGRASI: BERHASIL diterapkan ke database Supabase!");
